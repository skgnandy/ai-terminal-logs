...
detect() {
  ...
  # Asked of PM2, not guessed from $HOME. A service whose ecosystem file sets
  # out_file elsewhere is invisible to the ~/.pm2/logs guess, and nothing about
  # that failure is visible afterwards — see pm2_log_info in common.sh.
  PM2_LOG_GLOBS=()
  ...
  log "host=$HOST_NAME tz=$TZ_NAME docker=$HAS_DOCKER journal=$HAS_JOURNAL pm2_logs=${#PM2_LOG_GLOBS[@]} db_port=$PG_PORT"
  ...
  if [ ${#PM2_LOG_GLOBS[@]} -eq 0 ] && [ "$HAS_JOURNAL" -eq 0 ]; then
